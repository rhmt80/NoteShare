...
//    
//    func fetchNotes(forCollege collegeName: String, completion: @escaping ([String: [FireNote]]) -> Void) {
//        db.collection("pdfs")
//            .whereField("collegeName", isEqualTo: collegeName)
...
//                }
//
//                var groupedNotes: [String: [FireNote]] = [:]
...
//                    let subjectCode = data["subjectCode"] as? String ?? "Unknown"
//                    let subjectName = data["subjectName"] as? String ?? "Unnamed Subject"
//
...
//                            )
//
//                            if groupedNotes[subjectCode] != nil {
//                                groupedNotes[subjectCode]?.append(note)
//                            } else {
//                                groupedNotes[subjectCode] = [note]
//                            }
//                            group.leave()
//                        }
//                    }
...
//                  let _ = CGPDFDocument(provider) else {
//                let corruptError = NSError(domain: "PDFDownloadError",
//                                           code: -2,
//                                           userInfo: [NSLocalizedDescriptionKey: "Downloaded PDF is corrupt"])
//                completion(.failure(corruptError))
//                return
//            }
//            
...
//        College(name: "VIT", logo: UIImage(named: "iit_kanpur_logo")),
//        College(name: "KIIT", logo: UIImage(named: "nit_trichy_logo")),
//        College(name: "Manipal", logo: UIImage(named: "bits_pilani_logo")),
//        College(name: "LPU", logo: UIImage(named: "iit_madras_logo")),
//        College(name: "Amity", logo: UIImage(named: "iit_bombay_logo"))
//    ]
//    
//    // MARK: - UI Components
...
//    }
//    
//    private func fetchNotes(for collegeName: String) {
//        isLoading = true
//        activityIndicator.startAnimating()
//
//        FirebaseService.shared.fetchNotes(forCollege: collegeName) { [weak self] groupedNotes in
...
//                    // Show an alert if no notes are found
//                    let alert = UIAlertController(title: "No Notes", message: "No notes found for \(collegeName).", preferredStyle: .alert)
//                    alert.addAction(UIAlertAction(title: "OK", style: .default))
//                    self.present(alert, animated: true)
//                } else {
//                    // Navigate to NotesViewController with grouped notes
//                    let notesVC = NotesViewController()
//                    notesVC.configure(with: groupedNotes)
//                    notesVC.title = collegeName
...
//            fetchNotes(for: selectedCollege.name)
//        }
//    }
//    }
//
...
class FirebaseService {
    ...
    func fetchNotes(completion: @escaping ([FireNote], [String: [FireNote]]) -> Void){
        db.collection("pdfs").getDocuments { (snapshot, error) in
            ...
            snapshot?.documents.forEach { document in
                ...
                let pdfUrl = data["downloadURL"] as? String ?? ""
                let subjectCode = data["subjectCode"] as? String ?? "Unknown"
                _ = data["subjectName"] as? String ?? "Unnamed Subject"
...
class HomeViewController: UIViewController {
    ...
    private let colleges: [College] = [
        College(name: "SRMIST", logo: UIImage(named: "srmist_logo")),
        College(name: "VIT", logo: UIImage(named: "iit_kanpur_logo")),
        College(name: "KIIT", logo: UIImage(named: "nit_trichy_logo")),
        College(name: "Manipal", logo: UIImage(named: "bits_pilani_logo")),
        College(name: "LPU", logo: UIImage(named: "iit_madras_logo")),
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if collectionView == collegesCollectionView {
            ...
            FirebaseService.shared.fetchNotes { notesList, groupedNotes in
                let collegeNotes = notesList.filter { $0.author == selectedCollege.name }
                DispatchQueue.main.async {
                    self.notes = collegeNotes
                    self.notesCollectionView.reloadData()
                    self.activityIndicator.stopAnimating()
                }
            }
            ...
            let selectedNote = recommendedNotes[indexPath.item] // Use recommendedNotes
