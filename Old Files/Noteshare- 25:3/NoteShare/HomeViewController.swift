...
class NoteCollectionViewCell: UICollectionViewCell {
    ...
    override func layoutSubviews() {
        ...
        func configureWithCaching(with note: FireNote) {
            ...
            subjectNameLabel.text = note.category
            ...
            // Ensure favorite button is properly configured
            updateFavoriteButtonImage()
        }
...
class CollegeCell: UICollectionViewCell {
    ...
    private let collegeColors: [UIColor] = [
...
class HomeViewController: UIViewController, UICollectionViewDataSourcePrefetching {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        // Refresh favorites state when returning to Home
        fetchUserFavorites()
        
        // Refresh collegeNotesCache to ensure it has latest favorite states
    ...
    private func setupUI() {
            ...
            // Add activity indicator and loading UI to notes collection view
            notesCollectionView.addSubview(loadingView)
            ...
            NSLayoutConstraint.activate([
                ...
                notesCollectionView.heightAnchor.constraint(equalToConstant: 240), // Reduced from 280
                ...
                loadingView.centerXAnchor.constraint(equalTo: notesCollectionView.centerXAnchor),
                loadingView.centerYAnchor.constraint(equalTo: notesCollectionView.centerYAnchor),
    ...
    private func fetchNotesWithImprovedPerformance() {
        isLoading = true
        loadingView.isHidden = false
        activityIndicator.startAnimating()
        
        // Check cache first with compression
        if let cachedNotes: [FireNote] = CacheManager.shared.getCachedData(for: "recommendedNotes") {
            self.recommendedNotes = cachedNotes
            self.notesCollectionView.reloadData()
            self.activityIndicator.stopAnimating()
            self.loadingView.isHidden = true
            self.isLoading = false
            
            // Even if we use cache, refresh user favorites in the background
            self.fetchUserFavorites()
            
            // Prefetch images in the background
            self.prefetchImagesForNotes(cachedNotes)
            
            return
        }
        
        // First, fetch the metadata quickly
        FirebaseService.shared.fetchNotes { [weak self] fetchedNotes, _ in
            ...
            DispatchQueue.main.async {
                ...
                // Continue with recommended notes
            FirebaseService.shared.fetchRecommendedNotes { recommendedNotes in
                // Cache the recommended notes with compression
                try? CacheManager.shared.cacheData(recommendedNotes, for: "recommendedNotes")
                ...
                DispatchQueue.main.async {
                    ...
                    self.isLoading = false
                        ...
                        // Prefetch images in the background after displaying cells
    ...
    private func preloadCollegeData() {
        ...
        DispatchQueue.global(qos: .background).async { [weak self] in
            FirebaseService.shared.fetchNotes { [weak self] allNotes, groupedNotes in
                ...
                // Store basic college data immediately to show in UI
                DispatchQueue.main.async {
                    self.collegeNotesCache = groupedNotes
                    self.collegesCollectionView.reloadData()
                }
                
                // Continue processing metadata in background
                self.enhanceCollegeDataWithMetadata(groupedNotes: groupedNotes)
            }
        }
    }
...
extension HomeViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == notesCollectionView {
            ...
            // We no longer need this custom handler since the cell now posts the notification directly
            // Let the cell handle the favorite button taps with its own implementation
            return cell
        } else if collectionView == previouslyReadCollectionView {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: noteReuseIdentifier, for: indexPath) as! NoteCollectionViewCell
            ...
            // Configure cell with loading state if we have cached data
