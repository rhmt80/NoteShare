...
class FirebaseService1 {
    ...
    public func fetchPDFCoverImage(from urlString: String, completion: @escaping (UIImage?, Int) -> Void) {
        guard !urlString.isEmpty else {
    ...
    // Fetch favorite note IDs for the user
        public func fetchUserFavorites(userId: String, completion: @escaping ([String]) -> Void) {
...
class NoteCollectionViewCell1: UICollectionViewCell {
    ...
    private var currentColor: UIColor? {
        didSet {
            if let color = currentColor {
                bookBackgroundView.backgroundColor = color
                setGradientForColor(color)
            }
        }
    }
    ...
    override init(frame: CGRect) {
        ...
        // Set a default color to avoid white flash
        currentColor = colors[0] // Default to first color
    }
...
//        userNameLabel.text = nil
        noteId = nil
        isFavorite = false
        // Do NOT reset bookBackgroundView.backgroundColor or gradientLayer.colors to nil
        // Keep the last known color or set a default one
        if currentColor == nil {
            currentColor = colors[0] // Ensure there's always a color
        }
    }
...
//        userNameLabel.text = "By Loading..."
//        fetchUserName(userId: note.id) { [weak self] userName in
//            DispatchQueue.main.async {
...
//        }
        ...
        // Set color based on note ID (consistent across reuse)
        let colorIndex = abs(note.id.hashValue % colors.count)
        currentColor = colors[colorIndex] // This triggers didSet to update background and gradient
...
class SavedViewController: UIViewController, UIScrollViewDelegate, UITableViewDelegate, UITableViewDataSource, VNDocumentCameraViewControllerDelegate {
    ...
    private let uploadNotesButton: UIButton = {
            ...
            button.titleLabel?.font = .systemFont(ofSize: 16, weight: .medium) // Increased to 16 per HIG
            ...
            // Adjusted content insets for better fit (HIG recommends ~8-16pt padding)
        ...
        private let scanDocumentButton: UIButton = {
            ...
            // Adjusted content insets for better fit
            button.contentEdgeInsets = UIEdgeInsets(top: 10, left: 16, bottom: 10, right: 16)
    ...
    private func setupUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            uploadNotesButton.heightAnchor.constraint(greaterThanOrEqualToConstant: 44), // HIG minimum touch target
            ...
            scanDocumentButton.heightAnchor.constraint(greaterThanOrEqualToConstant: 44), // HIG minimum touch target
            
            // ScrollView (adjusted to be below buttons)
            scrollView.topAnchor.constraint(equalTo: uploadNotesButton.bottomAnchor, constant: 16), // Position below buttons
            ...
            favoriteNotesCollectionView.heightAnchor.constraint(equalToConstant: 265),
