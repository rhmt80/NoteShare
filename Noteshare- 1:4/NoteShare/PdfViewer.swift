...
class PDFViewerViewController: UIViewController, UIScrollViewDelegate, PDFViewDelegate {
    ...
    private let doubtsButton: UIButton = {
        let button = UIButton(type: .system)
        button.setTitle("AI Quiz", for: .normal)
        let sparklesConfig = UIImage.SymbolConfiguration(pointSize: 16, weight: .medium)
        let sparklesImage = UIImage(systemName: "sparkles", withConfiguration: sparklesConfig)
        button.setImage(sparklesImage, for: .normal)
    ...
    override func viewDidLoad() {
        ...
        pageIndicatorLabel.text = "Page 0 of 0"
    ...
    private func setupUI() {
        ...
        headerView.addSubview(pageIndicatorLabel)
        ...
        // Add info button
        metadataView.addSubview(infoButton)
    ...
    private func setupHeaderView() {
        ...
        NSLayoutConstraint.activate([
            ...
            pageIndicatorLabel.centerXAnchor.constraint(equalTo: headerView.centerXAnchor),
            pageIndicatorLabel.centerYAnchor.constraint(equalTo: headerView.centerYAnchor),
    ...
    private func setupConstraints() {
        ...
        // Create a variable height constraint for metadata view
        metadataHeightConstraint = metadataView.heightAnchor.constraint(equalToConstant: 420) // Increased height for more metadata spacing
        ...
        NSLayoutConstraint.activate([
            ...
            // Title Label Constraints - positioned below test knowledge container
            ...
            // Info Button Constraints - centered horizontally with increased size
            infoButton.topAnchor.constraint(equalTo: fileSizeContainer.bottomAnchor, constant: 24),
            infoButton.centerXAnchor.constraint(equalTo: metadataView.centerXAnchor),
            infoButton.widthAnchor.constraint(greaterThanOrEqualToConstant: 120),
            infoButton.heightAnchor.constraint(equalToConstant: 44),
            infoButton.bottomAnchor.constraint(lessThanOrEqualTo: metadataView.bottomAnchor, constant: -16)
        ])
        
        // Add tap gesture to collapse/expand metadata
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(handleMetadataTap))
        testKnowledgeContainer.addGestureRecognizer(tapGesture)
...
//                    self.handlePDFLoadingError("Could not load PDF document")
    ...
    private func updatePageIndicator() {
        if let document = pdfView.document, let currentPage = pdfView.currentPage {
            ...
            pageIndicatorLabel.text = "Page \(pageIndex + 1) of \(document.pageCount)"
        } else {
            pageIndicatorLabel.text = "Page 0 of 0"
        }
    }
    ...
    @objc private func infoButtonTapped() {
        ...
        UIView.animate(withDuration: 0.3) {
            self.infoButton.transform = self.infoButton.transform == .identity ?
    ...
    @objc private func handleMetadataTap() {
        toggleMetadataVisibility()
    }
    
    private func toggleMetadataVisibility() {
        isMetadataCollapsed.toggle()
        
        // Debug log for document ID
        if let docId = self.documentId {
//            print("📋 Toggling metadata for document ID: \(docId)")
        
        } else {
            print("📋 Toggling metadata (no document ID available)")
        }
        
        // Set appropriate height
        metadataHeightConstraint?.constant = isMetadataCollapsed ? collapsedMetadataHeight : 420
        
        // Set alpha for other elements in metadata view except for testKnowledgeContainer
        let elementsToHide: [UIView] = [
            titleLabel, dividerLine, courseContainer, courseIconView, courseNameLabel,
            universityContainer, universityIconView, universityNameLabel, infoButton,
            categoryContainer, categoryIconView, categoryLabel,
            uploaderContainer, uploaderIconView, uploaderLabel,
            uploadDateContainer, uploadDateIconView, uploadDateLabel,
            fileSizeContainer, fileSizeIconView, fileSizeLabel
        ]
        
        // Animate changes
        UIView.animate(withDuration: 0.3) {
            for element in elementsToHide {
                element.alpha = self.isMetadataCollapsed ? 0 : 1
            }
            self.view.layoutIfNeeded()
            
            // Apply test knowledge container styling
            self.applyTestKnowledgeContainerStyling()
            
            // Update PDF container size
            self.updatePDFContainerForCurrentState()
        }
    ...
    private func updatePDFContainerForCurrentState() {
        // Find the PDF container wrapper
        for view in contentStackView.arrangedSubviews where view != metadataView {
            if let wrapper = view as? UIView {
                // If metadata is collapsed, make PDF container bigger
                if isMetadataCollapsed {
                    wrapper.layer.cornerRadius = 0
                    
                    // Update container to fill screen when metadata is collapsed
                    for containerView in wrapper.subviews {
                    ...
                    // Restore rounded corners when metadata is expanded
                    for containerView in wrapper.subviews {
        ...
        // Ensure the test knowledge container maintains its shape
        applyTestKnowledgeContainerStyling()
...
extension PDFViewerViewController {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        // Auto-collapse metadata when scrolling down more than 20 points
        if scrollView.contentOffset.y > 20 && !isMetadataCollapsed {
            toggleMetadataVisibility()
        }
        // Auto-expand metadata when scrolling all the way to the top
        else if scrollView.contentOffset.y <= 0 && isMetadataCollapsed {
            toggleMetadataVisibility()
        }
        ...
        updateCurrentPageBasedOnScroll(scrollOffset: scrollView.contentOffset.y)
    ...
    // Add a new method to track the current visible page based on scroll position
    private func updateCurrentPageBasedOnScroll(scrollOffset: CGFloat) {
        guard let document = pdfView.document, document.pageCount > 0 else { return }
        
        // Calculate metadata height and account for any spacing
        let metadataHeight = isMetadataCollapsed ? collapsedMetadataHeight : metadataHeightConstraint?.constant ?? 270
        let metadataWithSpacing = metadataHeight + 16 // Add the spacing between metadata and PDF
        
        // If we've scrolled past the metadata, determine which page should be visible
        if scrollOffset > metadataWithSpacing {
            // Calculate the offset within the PDF content
            let offsetInPDF = scrollOffset - metadataWithSpacing
            
            // Account for the container and pdfView padding (25pt on top)
            let adjustedOffset = max(0, offsetInPDF - 25)
            
            // Find which page should be visible based on offset
            ...
            for i in 0..<document.pageCount {
                if let page = document.page(at: i) {
                    let pageSize = page.bounds(for: .mediaBox)
                    let pageHeight = pageSize.height * pdfView.scaleFactor
                    
                    // If this isn't the first page, add the gap between pages (32pt top + 32pt bottom)
                    if i > 0 {
                        accumulatedHeight += 64
                    }
                    
                    // Check if the current scroll position is within this page
                    if accumulatedHeight <= adjustedOffset && adjustedOffset < (accumulatedHeight + pageHeight) {
                        currentPage = i
                        break
                    }
                    ...
                    // If we've passed all checks, this is the current page
                    if adjustedOffset < accumulatedHeight {
                        currentPage = i
                        break
                    }
                }
            ...
            // Only update the indicator if the page has changed
            if let currentPDFPage = pdfView.currentPage,
               document.index(for: currentPDFPage) != currentPage,
               let newPage = document.page(at: currentPage) {
                // Update the PDFView's current page (will be visible when scrolling stops)
                pdfView.go(to: newPage)
                
                // Update the page indicator manually
                pageIndicatorLabel.text = "Page \(currentPage + 1) of \(document.pageCount)"
                
                print("Changed to page \(currentPage + 1)")
            } else if pdfView.currentPage == nil, let firstPage = document.page(at: currentPage) {
                // If no current page is set, set it
                pdfView.go(to: firstPage)
                pageIndicatorLabel.text = "Page \(currentPage + 1) of \(document.pageCount)"
            }
        } else {
            // At the top, make sure we're showing page 1
            if let firstPage = document.page(at: 0),
               (pdfView.currentPage == nil || document.index(for: pdfView.currentPage!) != 0) {
                pdfView.go(to: firstPage)
                pageIndicatorLabel.text = "Page 1 of \(document.pageCount)"
            }
        }
    }
}
