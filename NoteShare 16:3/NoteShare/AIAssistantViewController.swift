...
class AIPageViewController: ObservableObject {
    ...
    private func saveChatToFirestore(messages newMessages: [ChatMessage]) {
        ...
        // Add only the new messages to subcollection
    ...
    func loadSession(_ session: ChatSession) {
        ...
        db.collection("chats").document(session.id).getDocument { [weak self] chatSnapshot, error in
            ...
            // Set the PDF text first - CRITICAL for AI context
            DispatchQueue.main.async {
                print("Setting PDF text with \(pdfText.count) characters")
            ...
            // Then handle the PDF - first try with direct URL from chat data
            if let urlString = pdfUrlString, !urlString.isEmpty, let url = URL(string: urlString) {
                print("Using PDF URL directly from chat data: \(urlString)")
                let metadata = PDFMetadata(
                    id: pdfId,
                    url: url,
                    fileName: pdfName,
                ...
                DispatchQueue.main.async {
                    ...
                    self.selectedPDFMetadata = metadata
                    // Force hide welcome screen directly
                    if let scene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
                       let rootViewController = scene.windows.first?.rootViewController,
                       let advancedChatView = rootViewController as? UIHostingController<AdvancedChatView> {
                        print("Explicitly hiding welcome screen via UIKit")
                    }
                ...
                self.db.collection("pdfs").document(pdfId).getDocument { [weak self] snapshot, error in
                    ...
                    if let document = snapshot, document.exists, let data = document.data(),
                       ...
                       let fileSize = data["fileSize"] as? Int {
                        ...
                        DispatchQueue.main.async {
                            print("Setting PDF metadata from pdfs collection: \(fileName)")
                            self.selectedPDFMetadata = metadata
                            self.downloadPDF(from: url)
                        }
                    } else {
                        // If we can't find the PDF anywhere, show an error
    ...
    private func loadMessagesForChat(chatId: String) {
        ...
        db.collection("chats").document(chatId)
            ...
            .getDocuments { [weak self] snapshot, error in
                ...
                var messages = documents.compactMap { document -> ChatMessage? in
                    ...
                    let order = data["order"] as? Int ?? Int.max // Default to max if no order
                ...
                let hasOrderInfo = messages.contains { $0.order != Int.max }
                
                // Sort by order if available, otherwise by timestamp
                let sortedMessages: [ChatMessage]
                if hasOrderInfo {
                    // First try to sort by order field
                    sortedMessages = messages.sorted { $0.order < $1.order }
                    print("Sorted messages by order field")
                } else {
                    // If no order field, use a more robust approach
                    // First sort all messages by timestamp
                    let timestampSorted = messages.sorted { $0.timestamp < $1.timestamp }
                    
                    // Then try to naturally pair user and AI messages
                    var result: [ChatMessage] = []
                    var lastUserIndex = -1
                    
                    for (index, message) in timestampSorted.enumerated() {
                        if message.type == .user {
                            // Add the user message
                            result.append(message)
                            lastUserIndex = result.count - 1
                        } else if message.type == .ai {
                            // If this is an AI message and we've just added a user message,
                            // insert it after the user message
                            if lastUserIndex == result.count - 1 {
                                result.append(message)
                            } else {
                                // Otherwise, try to find a good spot based on timestamp
                                var inserted = false
                                for i in 0..<result.count {
                                    if i > 0 && result[i-1].type == .user && result[i].type != .ai &&
                                       message.timestamp > result[i-1].timestamp {
                                        result.insert(message, at: i)
                                        inserted = true
                                        break
                                    }
                                }
                                ...
                                if !inserted {
                                    // If we couldn't find a good spot, just append it
                                    result.append(message)
                                }
                            }
                        } else {
                            // For error messages or other types, just append
                            result.append(message)
                        }
                    }
                    
                    sortedMessages = result
                    print("Sorted messages using custom conversation flow logic")
                }
                
                DispatchQueue.main.async {
                    print("Loaded \(sortedMessages.count) messages in conversation order")
...
struct ChatHistoryView: View {
        ...
        viewModel.db.collection("chats")
            ...
            .addSnapshotListener { snapshot, error in
                ...
                for document in documents {
                    ...
                    if !pdfId.isEmpty {
                        ...
                        document.reference.collection("messages")
                            ...
                            .limit(to: 10) // Get just enough messages for preview
                            .getDocuments { messagesSnapshot, messagesError in
                                ...
                                var messages = messagesSnapshot?.documents.compactMap { messageDoc -> ChatMessage? in
                                    ...
                                    guard let content = messageData["content"] as? String,
                                          ...
                                          let type = MessageType(rawValue: typeString) else {
                                        print("Failed to parse message from chat history: \(messageData)")
                                        return nil
                                    }
                ...
                dispatchGroup.notify(queue: .main) {
                    ...
                    self.isLoading = false
                    print("Chat history updated with \(grouped.count) sections")
                }
            }
    }
    ...
    private func loadSession(_ session: ChatSession) {
        ...
        // Load the session
        self.viewModel.loadSession(session)
        
        // Dismiss the history view
        dismiss()
    }
